...
const solgrpc =
  process.env.solanagrpc ||
  "https://solana-yellowstone-grpc.publicnode.com:443"; // Free grpc endpoint
  ...
  listener.on("tradeparams", async (data) => {
    try {
      ...
      if (tokenIn && tokenOut && tradeVol) {
        ...
        console.log(
          chalk.green(`🚀 Trade coming in! 
                    - tokenIn: **${tokenIn}**
                    - tokenOut: **${tokenOut}**
                    - Trade Amount: **${tradeVol}**`)
        );
        const transaction = await swapRaydium(
          connection,
          data,
          keypair.publicKey
        );
        ...
        console.log({
          swapOutAmount: transaction.swapOutAmount,
          txBuffer: transaction.txBuffer,
        });
        // Transaction simulation
        const txSimulation = await connection.simulateTransaction(
...
async function createWssListener(connection, addresses, workerId) {
  if (!connection) {
    connection = new Connection(solrpc, {
...
function forkCPU(assignedAddresses) {
  ...
  newWorker.on("exit", (code, signal) => {
    console.error(
      `Worker ${newWorker.process.pid} exited with code ${code} and signal ${signal}`
    );
  ...
  try {
    ...
    if (cluster.isMaster) {
      ...
      for (let i = 0; i < numCPUs; i++) {
        const assignedAddresses = addresses.slice(
          i * Math.ceil(addresses.length / numCPUs),
          (i + 1) * Math.ceil(addresses.length / numCPUs)
        );
        forkCPU(assignedAddresses);
      }
      ...
      cluster.on("message", (worker, message) => {
        if (message.error) {
          console.error(
            `Error from worker ${worker.process.pid}:`,
            message.error
          );
        }
        if (message.log) {
      ...
      process.on("message", async ({ addresses, workerId }) => {
        ...
        try {
          let connection = new Connection(solrpc, {
            commitment: "confirmed",
            wsEndpoint: solwss,
            disableRetryOnRateLimit: false,
            async fetch(input, init) {
          ...
          const listener = await createGRPCListener(
            connection,
            addresses,
            workerId
          );
          ...
          console.log(error);
          process.send({
            error: `Worker ${workerId} failed: ${error.message}`,
          });
        }
      });
    }
