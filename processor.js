...
class logProcessor {
  ...
  async getTradeParamsWSS(connection, accountAddress, slot) {
    try {
      ...
      if (lastSig) sigConf.until = lastSig;
      // else
      sigConf.limit = 25;
      ...
      await Promise.all(
        tradetx.map(async (ele) => {
          try {
            ele.data = (
              await connection.getParsedTransaction(ele.signature, {
  ...
  async processEventGRPC(transactionData) {
    try {
      const accountAddress =
        transactionData.transaction.transaction.message.accountKeys[0];
