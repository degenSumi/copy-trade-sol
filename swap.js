...
async function swapRaydium(connection, tradeParams, from) {
  ...
  try {
    ...
    if (poolId.data.length === 0) return;
    ...
    if (!isValidAmm(poolInfo.programId)) return;
    ...
    return {
      swapOutAmount: out.minAmountOut,
      txBuffer: mainTx,
      allInstructions: tx.builder.allInstructions,
    };
  } catch (error) {
    console.log(error);
    throw error;
